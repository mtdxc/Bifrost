 ...
 * @project     : .
 * @file        : h264_file_buffer__producer.cpp
 * @description : TODO
 *******************************************************/
...
H264FileDataProducer::H264FileDataProducer(uint32_t ssrc, UvLoop *loop)
  ...
  std::ifstream file;
  file.open("../source_file/test.h264",
  ...
  if (file.is_open()) {
    ...
    std::cout << "empty test.h264 file, please copy a h264 file to "
                 "folder:\"worker/source_file/\"" << std::endl;
    exit(0);
  }
  file.seekg(0);
  ...
  read_frame_timer_ = new UvTimer(this, loop->get_loop());
  read_frame_timer_->Start(IntervalReadFrameMs, IntervalReadFrameMs);
}

H264FileDataProducer::~H264FileDataProducer() {
  delete[] buffer_;
  delete read_frame_timer_;
}
...
int H264FileDataProducer::GetH264FrameLen(int n_pos, size_t n_total_size, uint8_t *data) {
  int n_start = n_pos;
...
RtpPacketPtr H264FileDataProducer::CreateData() {
  auto packet_ite = rtp_packet_vec_.begin();
  if (packet_ite != rtp_packet_vec_.end() && cycle_bitrate_record_ > 0) {
...
bool H264FileDataProducer::ReadOneH264Frame() {
  if (read_offset < size_ - 4) {
    if (buffer_[read_offset] == 0x00 &&
        buffer_[read_offset + 1] == 0x00 &&
        buffer_[read_offset + 2] == 0x01) {
      start_code_offset_ = 3;

      frame_len_ = GetH264FrameLen(read_offset + start_code_offset_, size_, buffer_);
      payload_ = buffer_ + read_offset;

      frame_count_++;
      read_offset += 3;

      return false;
    } else if (buffer_[read_offset] == 0x00 &&
               buffer_[read_offset + 1] == 0x00 &&
               buffer_[read_offset + 2] == 0x00 &&
               buffer_[read_offset + 3] == 0x01) {
      start_code_offset_ = 4;
...
// 将帧切割为一个又一个的NALU
void H264FileDataProducer::H264FrameAnnexbSplitNalu(uint8_t *data, uint32_t len,
...
void H264FileDataProducer::ReadWebRTCRtpPacketizer() {
  ...
  if (fake_capture_timestamp_ == 0) {
    fake_capture_timestamp_ = uv_loop_->get_time_ms_int64();
  } else {
  ...
  auto nalu_type = PrintfH264Frame(frame_count_, frame_len_, (payload_ + start_code_offset_)[0]);

  int rtp_fragment_size = 0;
  // pair 的第一个为 length，第二个位 offset
  std::vector<std::pair<size_t, size_t>> fragment_infos;
  H264FrameAnnexbSplitNalu(payload_, frame_len_ + start_code_offset_,
      [&rtp_fragment_size, &fragment_infos](uint8_t *data, uint32_t len, uint32_t start_code_len) {
  ...
  // std::cout << Byte::bytes_to_hex(payload_,  frame_len_ + start_code_offset_) << std::endl;
  ...
  std::unique_ptr<webrtc::RtpPacketizer> packetizer = webrtc::RtpPacketizer::Create(
    type, rtc::MakeArrayView(payload_, frame_len_), limits,
  ...
  for (auto i = 0; i < num_packets; i++) {
    ...
    // 帧大小 + 普通头大小 + 拓展头
    webrtc::RtpPacketToSend packet(&webrtcExtension,
    ...
    // 转回bifrost的rtp格式
    RtpPacketPtr rtp_packet = std::make_shared<RtpPacket>(packet.data(), packet.size());
...
void H264FileDataProducer::OnTimer(UvTimer *timer) {
  if (timer == read_frame_timer_) {
    while (ReadOneH264Frame())
      ;
    ReadWebRTCRtpPacketizer();
  }
}
